...
macro ExpandProcEN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel)
{
    ...
    {   
        ...
        InsBufLine(hbuf, ln, "@szLine1@/* BugID: @szQuestion@     Author:@szMyName@,   Date:@sz@/@sz1@/@sz3@ ");
        szContent = Ask("Description")
        szLeft = cat(szLine1,"   Title : ");
        if(strlen(szLeft) > 70)
        {
            Msg("The right margine is small, Please use a new line")
            stop 
    ...
    else if (szCmd == "ab")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            ...
            {
                InsBufLine(hbuf, ln,     "@szLine1@/* BEGIN: BugID: @szQuestion@  Added by @szMyName@, @sz@/@sz1@/@sz3@");
                InsBufLine(hbuf, ln + 1, "@szLine1@          Title: @szContent@ */");
            }
            else
            {
                InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: BugID: @szQuestion@  Added by @szMyName@, @sz@/@sz1@/@sz3@ */");
    ...
    else if (szCmd == "ae")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* END  : BugID: @szQuestion@  Added by @szMyName@, @sz@/@sz1@/@sz3@ */");
        }
        else
        {
    ...
    else if (szCmd == "db")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            if(strlen(szContent) > 0)
            {
                InsBufLine(hbuf, ln,     "@szLine1@/* BEGIN: BugID: @szQuestion@  Deleted by @szMyName@, @sz@/@sz1@/@sz3@");
                InsBufLine(hbuf, ln + 1, "@szLine1@          Title: @szContent@ */");
            }
            else
            {
    ...
    else if (szCmd == "de")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* END  : BugID: @szQuestion@  Deleted by @szMyName@, @sz@/@sz1@/@sz3@ */");
        }
        else
        {
    ...
    else if (szCmd == "mb")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            if(strlen(szContent) > 0)
            {
                InsBufLine(hbuf, ln,     "@szLine1@/* BEGIN: BugID: @szQuestion@  Modified by @szMyName@, @sz@/@sz1@/@sz3@");
                InsBufLine(hbuf, ln + 1, "@szLine1@          Title: @szContent@ */");
            }
            else
            {
    ...
    else if (szCmd == "me")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* END  : BugID: @szQuestion@  Modified by @szMyName@, @sz@/@sz1@/@sz3@ */");
        }
        else
        {
...
macro AddMTKTrace()
{
	...
	szMyName = getreg(MYNAME)
	//Insert_Line SetBufSelText ( hbuf, "kal_prompt_trace(MOD_WAP, \"@szMyName@ %s, => %d\", __func__, __LINE__);")
	Insert_Line SetBufSelText ( hbuf, "logi(\"  \\n\");")
	stop
}
...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 2,  "  Copyright (C), 2004-2013, DEWAV Communication Technology Co., Ltd")
    ...
    /*Segon don't want to add func list*/
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), 2004-2013, 上海鼎为通讯科技有限公司")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
    ...
    */
    ...
    InsBufLine(hbuf, ln + 38, "")
    InsBufLine(hbuf, ln + 39, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 40, " * 外部函数原型说明                             *")
    InsBufLine(hbuf, ln + 41, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 42, "")
    InsBufLine(hbuf, ln + 43, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 44, " * 内部函数原型说明                             *")
    InsBufLine(hbuf, ln + 45, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 46, "")
    InsBufLine(hbuf, ln + 47, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 48, " * 函数实现                                     *")
    InsBufLine(hbuf, ln + 49, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 50, "")
    if(strlen(szContent) != 0)
    {
...
/*Segon add begin*/
macro ClearPromblemTitle()
{
   SetReg ("PT", "")
}

macro AddPromblemTitle()
{
    szContent = ASK("Please Input problem description ");
    if(szContent == "#")
    {
       szContent = ""
       SetReg ("PT", "")
